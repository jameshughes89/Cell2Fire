#!/usr/bin/env python3
"""
Resume an interrupted stochastic run by running only the missing jobs.
Reads matrix_results_stochastic.csv, finds completed (quad, wind_speed,
wind_dir, strategy, min_dist, seed) combos, and runs only what's missing.
Appends to the same CSV — no duplicates.
"""
...
IGNITIONS = {
    "centre": 39795, "NW": 19903, "NE": 20015, "SW": 59597, "SE": 59709,
}
WIND_SPEEDS = {
...
ACTIVE_STRATEGIES = [
    ...
    "cell5_hilly", "cell6_barriers", "cell7_cranked",
    "fuel_elevation", "neighbour_fuel", "shielded_ratio", "fuel_flank",
]
MIN_DISTS = [0, 2]
...
def load_completed():
    ...
    with open(OUT_CSV) as f:
        for r in csv.DictReader(f):
            completed.add((r["quadrant"], r["wind_speed"], r["wind_dir"],
                           r["strategy"], r["min_dist"], r["seed"]))
    return completed


def build_missing_jobs(completed):
    jobs = []
    for quad, cell_id in IGNITIONS.items():
        for wind_speed in WIND_SPEEDS:
            for wind_dir in WIND_DIRS:
                for seed in SEEDS:
                    key = (quad, wind_speed, str(wind_dir), "none", "0", str(seed))
                    if key not in completed:
                        jobs.append((quad, cell_id, wind_speed, wind_dir, "none", 0, seed))
                for strategy in ACTIVE_STRATEGIES:
                    for min_dist in MIN_DISTS:
                        for seed in SEEDS:
                            key = (quad, wind_speed, str(wind_dir), strategy,
                                   str(min_dist), str(seed))
                            if key not in completed:
...
def main():
    ...
    jobs = build_missing_jobs(completed)
    total = len(jobs)
    ...
    print(f"\nDone. {completed_count} runs, {failed} failed.  Results: {OUT_CSV}")
