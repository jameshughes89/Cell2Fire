#!/usr/bin/env python3
"""
Deterministic runs for the 12 doctrine/wildfireGP baseline strategies.

Same v2 protocol as run_v2_matrix.py (dogrib, 5 ignitions, 4 wind dirs,
3 wind speeds, md=0 and md=2, 5 seeds, ROS-CV=0.0).

Total: 5 × 4 × 3 × 12 × 2 × 5 = 7,200 runs.
Output: matrix_results_v2_doctrine.csv (separate from v2 and new-strats CSVs).
"""
...
OUT_CSV   = Path(os.environ.get("V2_DOCTRINE_OUT",
               str(REPO_ROOT / "matrix_results_v2_doctrine.csv")))
    ...
    for quad, cell_id in IGNITIONS.items():
        for wind_speed in WIND_SPEEDS:
            for wind_dir in WIND_DIRS:
                for strategy in DOCTRINE_STRATEGIES:
...
def main():
    jobs = build_jobs()
    total = len(jobs)
    print(f"Landscape: dogrib   ROS-CV: 0.0 (deterministic)")
    print(f"Doctrine strategies: {len(DOCTRINE_STRATEGIES)}  "
          f"Ignitions: {len(IGNITIONS)}  Wind dirs: {len(WIND_DIRS)}  "
          f"Wind speeds: {len(WIND_SPEEDS)}  Min dists: {MIN_DISTS}  Seeds: {len(SEEDS)}")
